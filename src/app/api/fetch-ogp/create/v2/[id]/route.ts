...
import connectDB from "@/app/utils/database";
import { CardModel } from "@/app/utils/schemaModels";
// useAuthUserを削除し、サーバーサイド向けの認証関数をインポート
import { getServerAuthUser } from "@/app/utils/serverAuth"; // このファイルは新たに作成する必要があります

export async function GET(req: NextRequest) {
  // Reactフックの代わりにサーバーサイド用の認証関数を使用
  const loginUserEmail = await getServerAuthUser(req);
